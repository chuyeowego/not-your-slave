...
| --- | --- | --- |
| AI gateway | `AI_GATEWAY_API_KEY` or `VERCEL_OIDC_TOKEN` in `.env.local` or `.env` | Live model replies, agent `said`/`thought`, `#chat` restore after reload |
...
**Photo scenarios** (`send-photo`, `photo-persist`, `say-limits`) need home image upload merged ([PR #25](https://github.com/chuyeowego/not-your-slave/pull/25)) — they fail against bare `main` until then.
...
$H doctor
$H drive conversation-log
$H drive text-persist    # #chat restore blocked without AI credential
